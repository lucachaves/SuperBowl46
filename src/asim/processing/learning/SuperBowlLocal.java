...
public class SuperBowlLocal extends PApplet{
	...
	public void draw(){
		...
		if(toRead == true){
			...
			try{
					...
					if(team == Team.GIANTS){									//if the team is the Giants, set fill color to RED
						fill(250,20,0,200);	
						countGiants++;
						radius = radiusTeams;
					}
					
					else if(team == Team.PATS){
						fill(0,20,250,250);										//otherwise set fill color to BLUE for Patriots
						countPats++;
						radius = radiusTeams;
					}
					
					else {														//team loyalty not known - make these GREEN and fade
						fill(0,150,0,150);
						countGeneral++;
						radius = radiusGeneric;
					}
